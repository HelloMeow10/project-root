...
async function forgotPassword(req, res) {
    ...
    if (!user)
        return res.status(200).json({ message: 'Si el email existe, se enviará un enlace para restablecer la contraseña.' });
...
async function resendVerificationEmail(req, res) {
    try {
        ...
        const user = await db_1.prisma.cliente.findUnique({
