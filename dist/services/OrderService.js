    ...
    async crearPedidoDesdeCarrito(idCliente, itemsConDetallesAdicionales, idDireccionFacturacion) {
        ...
        for (const itemFrontend of itemsConDetallesAdicionales) {
            ...
            if (((_a = itemBD.producto.tipoProducto) === null || _a === void 0 ? void 0 : _a.nombre.toLowerCase()) === 'vuelo') {
                ...
                if (itemFrontend.seleccion_asiento_fisico_id) {
                    ...
                    const asientoOcupado = await db_1.prisma.seleccionAsientoPasajero.findFirst({
                        where: {
                ...
                if (itemFrontend.selecciones_equipaje && Array.isArray(itemFrontend.selecciones_equipaje)) {
                    for (const equipaje of itemFrontend.selecciones_equipaje) {
                        if (!equipaje.id_opcion_equipaje)
                            continue;
                        ...
                        if (cantidadEquipaje <= 0)
                            throw new Error('La cantidad de equipaje debe ser positiva.');
        ...
        return db_1.prisma.$transaction(async (tx) => {
            ...
            for (const dataItemProcesado of itemsDataParaPedido) {
                const pedidoItemCreado = nuevoPedido.items.find(pi => pi.id_producto === dataItemProcesado.itemOriginal.id_producto &&
                ...
                if (dataItemProcesado.datosAsientoSeleccionado) {
                    ...
                    if (asientoOcupadoTx) {
                        ...
                        throw new Error(`El asiento ${asientoInfo === null || asientoInfo === void 0 ? void 0 : asientoInfo.fila}${asientoInfo === null || asientoInfo === void 0 ? void 0 : asientoInfo.columna} para el vuelo "${dataItemProcesado.itemOriginal.producto.nombre}" ya no está disponible (confirmado en transacción).`);
                    }
